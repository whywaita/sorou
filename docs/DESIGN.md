...
**sorou（揃う）** は、認証不要の日程調整サービスです。誰でもイベントを作成し、URL を共有するだけで参加者の出欠を収集できます。Cloudflare Workers + D1 上で動作し、エッジでの高速レスポンスとスケーラビリティを提供します。
...
**理由**: 認証がないため「同名＝同一人物」というヒューリスティックに依存する。
