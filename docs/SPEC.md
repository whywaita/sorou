...
| 項目 | 既存サービス | sorou (本サービス) |
|------|---------------|-------------------------------|
| 認証 | 不要 | 不要 |
| インフラ | Laravel (VPS) | Cloudflare Workers (エッジ) |
| UI | Vue.js SPA | SSR (Hono JSX) — クライアントJS最小限 |
| API | なし（スクレイピング） | RESTful JSON API |
| 拡張性 | クローズド | オープンソース |
| レイテンシ | 東京リージョン | グローバルエッジ |
