          ...
          <table class="w-full text-sm">
            ...
            <tbody>
              {ev.candidates.map((c) => {
                ...
                const isBest = bestCandidateIds(ev.candidates, responses).has(
                  c.id,
                );
                return (
                  <tr
                    class={`border-b border-slate-100 ${isBest ? "bg-emerald-50" : ""}`}
                  >
                    <td class="px-4 py-3 font-medium whitespace-nowrap">
                      {escapeHtml(c.date)}
                    </td>
                    {responses.map((r) => {
                      const st = r.statuses.find((s) => s.candidateId === c.id);
                      return (
                    ...
                    <td class="px-4 py-3 text-center text-xs text-slate-500">
                      {count.yes > 0 && <span class="mr-1">{count.yes}〇</span>}
                      {count.maybe > 0 && (
          ...
          {ev.candidates.map((c, i) => (
            <div class="flex items-center gap-3">
              ...
              <div class="flex gap-4">
                {(["〇", "△", "×"] as const).map((s) => {
                  const checked =
                    edit?.statuses[c.id] === s || (!edit && s === "×") || false;
                  const colors = {
      ...
      <div class="bg-slate-100 rounded-lg p-4 text-sm">
        ...
        <div class="flex items-center gap-2">
          ...
          <button
            ...
            class="px-3 py-2 bg-white border border-slate-300 rounded text-sm hover:bg-slate-50 whitespace-nowrap"
          >
            コピー
          </button>
        </div>
